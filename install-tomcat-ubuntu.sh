...
echo "*****Downloading Apache Tomcat 9.0.102 version"
cd /opt
sudo systemctl stop tomcat 2>/dev/null
sudo rm -rf apache* tomcat*
sudo mkdir -p /opt/tomcat
sudo wget -q https://archive.apache.org/dist/tomcat/tomcat-9/v9.0.102/bin/apache-tomcat-9.0.102.tar.gz
sudo tar xf apache-tomcat-9.0.102.tar.gz -C /opt/tomcat 1>/dev/null
...
cd - 1>/dev/null
sudo cp context.xml /opt/tomcat/apache-tomcat-9.0.102/webapps/manager/META-INF/context.xml
sudo cp context.xml /opt/tomcat/apache-tomcat-9.0.102/webapps/host-manager/META-INF/context.xml
sudo cp tomcat-users.xml /opt/tomcat/apache-tomcat-9.0.102/conf/tomcat-users.xml
echo "            -> Done"
